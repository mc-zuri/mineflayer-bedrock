...
type DeserializedPacket<K extends keyof PacketParamsMap = keyof PacketParamsMap> =
  K extends any
    ? {
        name: K;
...
declare module "bedrock-protocol" {
  interface Player {
    on(
      name: "serverbound",
      cb: (name: string, des: PacketData) => void
    ): unknown;
    on(
      name: "clientbound",
      cb: (name: string, des: PacketData) => void
...
type PacketParamsMap = {
  [K in keyof protocolTypes.BedrockPacketEventMap]: protocolTypes.BedrockPacketEventMap[K] extends (
    packet: infer P
  ) => void
    ? P
    : never;
...
type PacketData<K extends keyof PacketParamsMap = keyof PacketParamsMap> =
  K extends any
    ? {
        name: K;
        buffer: Buffer;
