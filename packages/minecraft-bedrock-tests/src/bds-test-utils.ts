...
import { type BDSServer, type BDSServerOptions, startBDSServer } from './bds-server.ts';

/**
 * Run a test function with a managed BDS server instance.
 * Server is automatically started before and stopped after the test.
 */
export async function withBDSServer(testFn: (server: BDSServer) => Promise<void>, options?: BDSServerOptions): Promise<void> {
  const server = await startBDSServer(options);
  try {
    await testFn(server);
  } finally {
    await server.stop();
...
/**
 * Connect a mineflayer bot to the BDS server.
 */
export async function connectBotToBDS(server: BDSServer, botOptions?: Partial<BotOptions>): Promise<Bot> {
 ...
 */
export async function giveItem(server: BDSServer, player: string, item: string, count = 1): Promise<void> {
 ...
 */
export async function clearInventory(server: BDSServer, player: string): Promise<void> {
 ...
 */
export async function teleportPlayer(server: BDSServer, player: string, x: number, y: number, z: number): Promise<void> {
 ...
 */
export async function setGamemode(server: BDSServer, player: string, mode: 'survival' | 'creative' | 'adventure' | 'spectator'): Promise<void> {
 ...
 */
export async function setBlock(server: BDSServer, x: number, y: number, z: number, block: string): Promise<void> {
 ...
 */
export async function fill(server: BDSServer, x1: number, y1: number, z1: number, x2: number, y2: number, z2: number, block: string): Promise<void> {
 ...
 */
export async function killEntities(server: BDSServer, entityType: string): Promise<void> {
 ...
 */
export async function pingBehaviorPack(server: BDSServer): Promise<boolean> {
  try {
 ...
 * Requires test_helper behavior pack.
 */
export async function getServerInventory(server: BDSServer, playerName?: string): Promise<ServerInventoryItem[]> {
 ...
 * Requires test_helper behavior pack.
 */
export async function getServerPlayerState(server: BDSServer, playerName?: string): Promise<ServerPlayerState> {
  const message = playerName ?? '';
 ...
 * Requires test_helper behavior pack.
 */
export async function getServerBlockInventory(server: BDSServer, x: number, y: number, z: number): Promise<ServerBlockInventory> {
 ...
 * Requires test_helper behavior pack.
 */
export async function clearInventoryViaBehaviorPack(server: BDSServer, playerName?: string): Promise<void> {
  const message = playerName ?? '';
