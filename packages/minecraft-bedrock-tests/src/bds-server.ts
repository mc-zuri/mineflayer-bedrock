...
function getDefaultOptions(): BDSServerOptions {
  const workerId = getWorkerId();
...
function updateServerProperties(bdsPath: string, opts: Required<BDSServerOptions>): void {
...
export async function startBDSServer(options?: BDSServerOptions): Promise<BDSServer> {
  const opts = { ...getDefaultOptions(), ...options } as Required<BDSServerOptions>;
  const workerId = getWorkerId();
